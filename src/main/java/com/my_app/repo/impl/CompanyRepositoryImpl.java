...
public class CompanyRepositoryImpl implements CompanyRepository {
	...
	private Company create(Company company) {
		try (final PreparedStatement stmt = this.conn
			...
			stmt.setString(1, CompanyUtils.normalizeCompanyName(company.getName()));
	...
	private Company update(Company company) {
		try (final PreparedStatement stmt = this.conn.prepareStatement(
				"UPDATE \"COMPANY\" SET NAME = ?,ADDRESS = ?, VATNUMBER = ?, CITY_ID = ?  WHERE ID = ?")) {
	...
	public Company findByName(String name) {
		try (final PreparedStatement stmt = this.conn
			...
			stmt.setString(1, CompanyUtils.normalizeCompanyName(name));
