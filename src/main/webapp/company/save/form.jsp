...
<title>Company - MyApp</title>
<jsp:include page="/incl/styles.jsp" />
</head>
<body>
    <jsp:include page="/incl/header.jsp" />
    <main class="container">
        <h1>${form.newCompany ? 'New' : 'Update'} Company ${form.newCompany ? '' : form.companyName}</h1>
        <c:choose>
        ...
        <div class="row">
            <div class="col-xs-12 col-md-4">
                <form id="company-save-form" action="${contextPath}/company/save.do" method="POST">
                    <input type="hidden" name="action" value="submit" />
                    <input type="hidden" name="formInit" value="${form.formInit}" />
                    <input type="hidden" id="company-id" name="companyId" value="${form.userId}" />
                    <input type="hidden" id="company-original-companyName" name="originalCompanyName" value="${form.originalCompanyName}" />

                    <div class="form-group ${actionErrors.get('name').hasNext() ? 'has-error' : (validated ? 'has-success' : '')}">
                        <label for="company-name">Name</label>
                        <input id="company-name" type="text" class="form-control" name="name" value="${form.companyName}" placeholder="CompanyName" aria-label="CompanyName" maxlength="100" required />
                    ...
                    <div class="form-group ${actionErrors.get('address').hasNext() ? 'has-error' : (validated ? 'has-success' : '')}">
                        ...
                        <input id="company-address" type="text" class="form-control" name="address" value="${form.address}" placeholder="Address" aria-label="Address" maxlength="200" required />
                        <div class="help-block">
                            <html:errors property="address" />
                        </div>
                    </div>
                    
                    <div class="form-group ${actionErrors.get('iva').hasNext() ? 'has-error' : (validated ? 'has-success' : '')}">
                        <label for="company-iva">Iva</label>
                        <input id="company-iva" type="text" class="form-control" name="iva" value="${form.iva}" placeholder="Iva" aria-label="Iva" maxlength="10" required />
                        <div class="help-block">
                            <html:errors property="iva" />
                        </div>
                    </div>
                    ...
                    <a class="btn btn-default" href="${contextPath}/company.do">Back</a>
